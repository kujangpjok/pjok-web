...
export default function IdentitasPage(){
  ...
  const onSubmit=(e)=>{
    ...
    for(const f of FIELDS){
      if(!data[f.key] || String(data[f.key]).trim()===""){ alert(`${f.label} wajib diisi`); return; }
    }
    setSaving(true);
    saveIdentitas(data);
    setSaving(false);
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          {FIELDS.map(f=>(
            <div key={f.key} className="flex flex-col gap-1">
              ...
              <input className="px-3 py-2 rounded border border-slate-300 bg-white text-slate-900 outline-none"
                value={data[f.key]||""} onChange={e=>setData({...data,[f.key]:e.target.value})} required />
            </div>
          ))}
          <button disabled={saving} className="px-4 py-2 rounded bg-blue-600 text-white hover:brightness-110 disabled:opacity-60" type="submit">
